...
import { User, Phone, MapPin, Calendar, Shield, Save, X, AlertTriangle, CheckCircle, Users, Briefcase, Heart, DollarSign, FileText, Home, Gift } from 'lucide-react';
import { type Beneficiary } from '../data/mockData';
import { useErrorLogger } from '../utils/errorLogger';
import { Button, Card, Input, Badge } from './ui';
import { referralService } from '../services/supabaseService';
...
export default function BeneficiaryForm({ beneficiary, onSave, onCancel }: BeneficiaryFormProps) {
  ...
  useEffect(() => {
    if (beneficiary) {
      setFormData({
        ...
        referredByCode: beneficiary.referredByCode || '',
      });
    }
  }, [beneficiary]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.referredByCode) {
      try {
        const validation = await referralService.validateAndUseReferralCode(
          formData.referredByCode, 
          beneficiary?.id || 'new-beneficiary'
        );
        
        if (!validation.valid) {
          setOperationError('رمز الإحالة غير صالح أو منتهي الصلاحية');
          return;
        }
        
        logInfo(`تم استخدام رمز الإحالة: ${formData.referredByCode}`, 'BeneficiaryForm');
      } catch (error) {
        setOperationError('خطأ في التحقق من رمز الإحالة');
        logError(error as Error, 'BeneficiaryForm');
        return;
      }
    }
        ...
        <Card>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <Input
              ...
              onChange={(e) => handleInputChange('referredByCode', e.target.value.toUpperCase())}
              placeholder="مثال: MT564"
            />
            
            <div className="bg-green-50 p-4 rounded-xl border border-green-200">
              <h4 className="font-medium text-green-800 mb-2">فوائد استخدام رمز الإحالة:</h4>
              <ul className="text-sm text-green-700 space-y-1">
                <li>• تسهيل عملية التسجيل</li>
                <li>• المحيل يتحمل رسوم الإحالة (5 ₪)</li>
                <li>• تسريع عملية التسجيل</li>
              </ul>
            </div>
          </div>
        </Card>

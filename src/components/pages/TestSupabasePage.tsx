...
import { mockBeneficiaries, mockPackages, mockTasks, mockAlerts, mockRoles, mockSystemUsers, calculateStats } from '../../data/mockData';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          {/* Packages Sample */}
          <div className="bg-green-50 p-4 rounded-xl border border-green-200">
            <h4 className="font-medium text-green-800 mb-3">عينة من الطرود ({packages.length} إجمالي):</h4>
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {packages.slice(0, 5).map((pkg, index) => (
                <div key={pkg.id} className="bg-white p-3 rounded-lg flex justify-between items-center">
                  <div>
                    <p className="font-medium text-gray-900">{pkg.name}</p>
                    <p className="text-sm text-gray-600">{pkg.type}</p>
                  </div>
                  <span className="text-sm text-gray-600">{pkg.value} ₪</span>
                </div>
              ))}
            </div>
          </div>
